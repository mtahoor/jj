 ...
 * 
 * This script creates and manages toast notifications that appear at the top-right
 * corner of the screen and automatically disappear after a few seconds.
 */
...
function createToastContainer() {
  ...
  if (!container) {
    ...
    container.className = 'fixed top-4 right-4 z-50 flex flex-col gap-2 max-w-xs w-full';
...
function createToast(message, type = 'info') {
  ...
  toast.id = id;
  toast.className = `toast-message flex items-center w-full p-4 mb-2 rounded-lg shadow-md transition-all duration-300 transform translate-x-full opacity-0 ${bgColor} ${textColor} ${borderColor}`;
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  djangoMessages.forEach((messageElement, index) => {
